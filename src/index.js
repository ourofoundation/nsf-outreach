...
program
  ...
  .action(async (options) => {
    ...
    if (years.length === 0) {
      ...
      console.log(`Create folders like ${chalk.cyan('awards/2024/')} and add NSF JSON files.\n`);
      return;
    }
    ...
    let totalStats = { total: 0, withEmail: 0, unprocessed: 0, readyToGenerate: 0 };
    ...
    for (const year of years) {
      ...
      console.log(`   With email: ${chalk.green(stats.withEmail)} | Without: ${chalk.yellow(stats.withoutEmail)}`);
      console.log(`   Unprocessed: ${chalk.blue(stats.unprocessed)}`);
      console.log(`   Ready to generate: ${chalk.green(stats.readyToGenerate)}\n`);
    }
    ...
    if (years.length > 1) {
      ...
      console.log(`   Ready to generate: ${chalk.green(totalStats.readyToGenerate)}\n`);
    }
  });
...
program
  ...
  .action(async (options) => {
    ...
    if (awards.length === 0) {
      console.log(chalk.yellow('No unprocessed awards with valid email found.'));
      console.log(chalk.dim('Run `nsf-outreach scan` to see available awards.\n'));
      return;
    }
    
    console.log(`Found ${chalk.cyan(awards.length)} unprocessed awards with email`);
    ...
    const results = await generateEmails(awards, {
      ...
      onProgress: ({ current, total, awardId }) => {
        console.log(chalk.dim(`[${current}/${total}]`) + ` Generating for ${chalk.cyan(awardId)}...`);
      }
    });
    ...
    if (results.generated.length > 0) {
      console.log(chalk.green(`✅ Generated ${results.generated.length} drafts`));
    ...
    console.log(chalk.dim('Next: Review drafts in drafts/ folder, then move approved ones to approved/\n'));
  });
...
program
  ...
  .action(() => {
    ...
    console.log(`   ${chalk.yellow('Drafts:')}    ${drafts} emails awaiting review`);
    console.log(`   ${chalk.blue('Approved:')}  ${approved} emails ready to send`);
    ...
    if (total > 0) {
      ...
      console.log(`   Progress: ${chalk.cyan(progress + '%')} (${sent}/${total} processed)\n`);
    }
    
    if (drafts > 0) {
      console.log(chalk.dim(`Next: Review emails in ${DIRS.drafts}/ and move to ${DIRS.approved}/\n`));
    } else if (approved > 0) {
      console.log(chalk.dim(`Next: Run 'nsf-outreach send' to deliver approved emails\n`));
    }
  });
...
program
  ...
  .action(async (options) => {
    ...
    if (approved.length === 0) {
      ...
      console.log(chalk.dim(`Move reviewed drafts to ${DIRS.approved}/ first.\n`));
      return;
    }
    
    console.log(chalk.bold(`\n📤 Sending Emails${dryRun ? ' (DRY RUN)' : ''}\n`));
    console.log(`   Found ${chalk.cyan(approved.length)} approved emails`);
    console.log(`   Sending up to ${chalk.cyan(limit)} with ${chalk.cyan(delay + 's')} delay\n`);
    ...
    const results = await sendApprovedEmails({
      ...
      onProgress: ({ current, total, awardId, recipient }) => {
        ...
        console.log(chalk.dim(`[${current}/${total}]`) + ` ${status} ${chalk.cyan(recipient)}`);
      }
    });
...
program
  .command('preview')
  .description('Preview a draft email')
  .argument('<award-id>', 'Award ID to preview')
  .option('-f, --folder <folder>', 'Folder to look in', 'drafts')
  .action((awardId, options) => {
    ...
    console.log(chalk.dim('─'.repeat(60)));
    console.log(`${chalk.dim('Award:')} ${email.award_id} - ${email.award_title}`);
